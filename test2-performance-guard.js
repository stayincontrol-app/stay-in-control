...
window.setInterval=function(fn,delay,...args){
  try{
    ...
    if(ms===300&&src.includes('ap207-authenticated')&&src.includes('properties()')){
      ...
      return nativeSetInterval(fn,1500,...args);
    }
    if(ms===120&&src.includes('newPropertyButton')){
      queueMicrotask(()=>{try{fn(...args)}catch(e){console.error(e)}});
      return nativeSetInterval(fn,1000,...args);
    }
  }catch{}
...
let lastInputAt=0;document.addEventListener('input',()=>{lastInputAt=Date.now()},{capture:true,passive:true});function syncShell(){if(Date.now()-lastInputAt<180)return requestAnimationFrame(syncShell);document.querySelectorAll('[data-copy]').forEach(el=>{const source=document.getElementById(el.dataset.copy||'');if(source&&el.textContent!==source.textContent)el.textContent=source.textContent});const month=document.querySelector('.t2-pro-period .month'),source=document.getElementById('monthLabel');if(month&&source&&month.textContent!==source.textContent)month.textContent=source.textContent}
