...
function isCompleted(date,year){if(!(date instanceof Date)||Number.isNaN(date.getTime()))return false;const now=new Date();const end=new Date(year,11,31,23,59,59,999);const cutoff=year<now.getFullYear()?end:new Date(now.getFullYear(),now.getMonth(),1);return date.getFullYear()===year&&date<cutoff;}
function filterVisibleCards(month,year){
 const all=String(month)==='all';
 const panel=document.querySelector('[data-screen-panel="reservations"]');if(panel&&!panel.hidden){panel.querySelectorAll('#reservations .booking').forEach(card=>{const text=card.textContent||'';const dates=[...text.matchAll(/(\d{2})\/(\d{2})\/(20\d{2})/g)];if(!dates.length){card.style.display='';return;}const d=dates[0],dt=new Date(Number(d[3]),Number(d[2])-1,Number(d[1]));card.style.display=all?(isCompleted(dt,year)?'':'none'):((Number(d[2])===Number(month)&&Number(d[3])===year)?'':'none');});}
 const exp=document.querySelector('[data-screen-panel="expenses"]');if(exp&&!exp.hidden){exp.querySelectorAll('#expenses .expense-item,#expenses .booking').forEach(card=>{const text=card.textContent||'';const d=text.match(/(\d{2})\/(\d{2})\/(20\d{2})/);if(!d){card.style.display='';return;}const dt=new Date(Number(d[3]),Number(d[2])-1,Number(d[1]));card.style.display=all?(isCompleted(dt,year)?'':'none'):((Number(d[2])===Number(month)&&Number(d[3])===year)?'':'none');});}
}
function sync(month,year){
 ...
 if(String(month)!=='all'){
  ...
  const label=document.getElementById('monthLabel');if(label)label.textContent=`Todos os meses completos de ${year}`;
 }
 filterVisibleCards(month,year);
}
function restoreHomeAllData(){const home=document.querySelector('[data-screen-panel="home"]');if(!home||home.hidden)return;document.querySelectorAll('#reservations .booking,#expenses .expense-item,#expenses .booking').forEach(card=>card.style.display='');if(typeof window.renderAll==='function'){try{window.renderAll();}catch(e){}}window.dispatchEvent(new CustomEvent('staycontrol:home-show-all'));}
function updateVisibility(){const wrap=document.querySelector('.global-period-selector');if(!wrap)return;const home=document.querySelector('[data-screen-panel="home"]');const homeVisible=home&&!home.hidden;wrap.style.display=homeVisible?'none':'';if(homeVisible)setTimeout(restoreHomeAllData,0);else{const ms=document.getElementById('globalMonthSelector'),ys=document.getElementById('globalYearSelector');if(ms&&ys)setTimeout(()=>filterVisibleCards(ms.value,Number(ys.value)),0);}}
function inject(){
 ...
 const wrap=document.createElement('div');wrap.className='global-period-selector';wrap.innerHTML=`<label for="globalMonthSelector">Selecionar mês</label><div class="global-period-row"><select id="globalMonthSelector" aria-label="Selecionar mês"><option value="all">Todos os meses — finalizados</option>${MONTHS.map((m,i)=>`<option value="${i+1}">${m}</option>`).join('')}</select><select id="globalYearSelector" aria-label="Selecionar ano"></select></div>`;
 ...
 const go=()=>sync(ms.value==='all'?'all':Number(ms.value),Number(ys.value));ms.addEventListener('change',go);ys.addEventListener('change',go);
