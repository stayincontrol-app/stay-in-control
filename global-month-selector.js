...
function filterVisibleCards(month,year){
 const panel=document.querySelector('[data-screen-panel="reservations"]');if(panel){
   panel.querySelectorAll('#reservations .booking').forEach(card=>{const text=card.textContent||'';const dates=[...text.matchAll(/(\d{2})\/(\d{2})\/(20\d{2})/g)];if(!dates.length){card.style.display='';return;}const d=dates[0];card.style.display=(Number(d[2])===month&&Number(d[3])===year)?'':'none';});
 }
 const exp=document.querySelector('[data-screen-panel="expenses"]');if(exp){
   exp.querySelectorAll('#expenses .expense-item,#expenses .booking').forEach(card=>{const text=card.textContent||'';const d=text.match(/(\d{2})\/(\d{2})\/(20\d{2})/);if(!d){card.style.display='';return;}card.style.display=(Number(d[2])===month&&Number(d[3])===year)?'':'none';});
 }
}
function sync(month,year){
 localStorage.setItem('stay-control-selected-period',JSON.stringify({month,year}));
 const cm=document.getElementById('calendarMonth'),cy=document.getElementById('calendarYear');
 if(cm){cm.value=String(month);dispatch(cm);} if(cy){cy.value=String(year);dispatch(cy);}
 const rm=document.getElementById('reportMonth'),ry=document.getElementById('reportYear');
 if(rm){rm.value=String(month);dispatch(rm);} if(ry){ry.value=String(year);dispatch(ry);}
...
function inject(){
 ...
 const ms=wrap.querySelector('#globalMonthSelector'),ys=wrap.querySelector('#globalYearSelector');
 const now=new Date();for(let y=now.getFullYear()-3;y<=now.getFullYear()+3;y++){const o=document.createElement('option');o.value=o.textContent=String(y);ys.append(o);}
