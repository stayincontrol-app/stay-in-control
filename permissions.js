(function (root, factory) {
  ...
  function hasPermission(user, permission) {
    const effective = effectiveUser(user);
    if (!effective || effective.active === false || !PERMISSIONS[effective.role]) return false;
    return PERMISSIONS[effective.role].includes('*') || PERMISSIONS[effective.role].includes(permission);
  }
  ...
  function injectAuthStyles() {
    ...
    style.textContent = `
      body.ap207-auth-pending > main, body.ap207-auth-pending > nav { visibility: hidden !important; }
      .ap207-auth-overlay { position: fixed; inset: 0; z-index: 99999; display: grid; place-items: center; padding: 22px; background: #f4f7fb; font-family: -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
      .ap207-auth-card { width: min(100%, 430px); background: white; border-radius: 22px; padding: 26px; box-shadow: 0 18px 55px rgba(20,35,60,.16); }
      .ap207-auth-card h1 { margin: 0 0 5px; font-size: 28px; }
      .ap207-auth-card p { color: #64748b; line-height: 1.45; }
      .ap207-auth-card label { display:block; margin: 15px 0 6px; font-size: 13px; font-weight: 700; color:#334155; }
      .ap207-auth-card input { width:100%; box-sizing:border-box; padding:13px 14px; border:1px solid #d7dee8; border-radius:12px; font-size:16px; }
      .ap207-auth-actions { display:grid; gap:10px; margin-top:18px; }
      .ap207-auth-button { border:0; border-radius:12px; padding:13px 16px; font-size:15px; font-weight:800; cursor:pointer; background:#111827; color:white; }
      .ap207-auth-button.secondary { background:#eef2f7; color:#172033; }
      .ap207-auth-message { min-height:22px; margin-top:12px; color:#b42318 !important; font-size:14px; }
      .ap207-auth-note { margin-top:16px !important; font-size:12px; }
      body.ap207-authenticated label[for="userSelector"], body.ap207-authenticated #userSelector { display:none !important; }
    `;
  ...
  function createAuthOverlay() {
    let overlay = document.getElementById('ap207AuthOverlay');
    if (overlay) return overlay;
    overlay = document.createElement('div');
    overlay.id = 'ap207AuthOverlay';
    overlay.className = 'ap207-auth-overlay';
    overlay.innerHTML = `
      <section class="ap207-auth-card" aria-labelledby="ap207LoginTitle">
        <p style="margin:0 0 4px;font-weight:800;color:#2563eb">Stay in Control 1.0</p>
        <h1 id="ap207LoginTitle">Acessar painel</h1>
        <p>Entre com seu e-mail e senha. Cada usuário verá somente o que sua permissão autorizar.</p>
        <form id="ap207LoginForm">
          <label for="ap207Email">E-mail</label>
          <input id="ap207Email" type="email" autocomplete="email" required>
          <label for="ap207Password">Senha</label>
          <input id="ap207Password" type="password" autocomplete="current-password" minlength="6" required>
          <div class="ap207-auth-actions">
            <button class="ap207-auth-button" type="submit">Entrar</button>
            <button class="ap207-auth-button secondary" id="ap207CreateAccount" type="button">Criar minha conta</button>
          </div>
        </form>
        <p id="ap207AuthMessage" class="ap207-auth-message" role="alert"></p>
        <p class="ap207-auth-note">O primeiro cadastro deste sistema será criado como Super Administrador. Os próximos cadastros entram como Proprietário até serem vinculados pelo administrador.</p>
      </section>`;
  ...
  async function loadAuthenticatedProfile(client, user) {
    ...
    if (profile.role !== ROLES.SUPER_ADMIN) {
      ...
      propertyIds = (accessRows || []).map((row) => row.property_id);
    }
  ...
  async function bootstrapAuth() {
    ...
    const message = overlay.querySelector('#ap207AuthMessage');
    ...
    try {
      ...
      if (session?.user) {
        const authData = await loadAuthenticatedProfile(client, session.user);
        const changed = JSON.stringify(cachedAuth) !== JSON.stringify(authData);
      ...
      overlay.querySelector('#ap207LoginForm').addEventListener('submit', async (event) => {
        ...
        const password = overlay.querySelector('#ap207Password').value;
        const { data, error } = await client.auth.signInWithPassword({ email, password });
        if (error) { message.textContent = 'E-mail ou senha inválidos.'; return; }
        try {
          const authData = await loadAuthenticatedProfile(client, data.user);
          writeAuthCache(authData.profile, authData.propertyIds);
          location.reload();
        } catch (profileError) {
          message.textContent = profileError.message || 'Não foi possível carregar as permissões.';
        }
      });
      ...
      message.textContent = error?.message || 'Não foi possível iniciar o login.';
      clearAuthCache();
    }
  }
