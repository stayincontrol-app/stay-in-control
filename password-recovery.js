(() => {
  "use strict";
  const RECOVERY_URL =
    "https://stay-in-control-git-main-marcelinhone-9670s-projects.vercel.app/invite.html?recovery=1";
  function install() {
    ...
    button.onclick = async () => {
      ...
      if (!email) {
        message.textContent =
          "Digite seu e-mail acima para solicitar a redefinição da senha.";
        return;
      }
      ...
      try {
        ...
        if (error) {
          ...
          message.textContent =
            d.includes("rate") || d.includes("limit")
        ...
        message.textContent =
          "✅ Solicitação enviada. Verifique a Caixa de Entrada, Spam e Lixo Eletrônico. O recebimento do e-mail pode levar alguns minutos.";
        let seconds = 60;
        const original = button.textContent;
        const tick = () => {
          button.textContent = `Enviar novamente (${seconds}s)`;
          seconds -= 1;
          if (seconds < 0) {
            clearInterval(timer);
        ...
        message.style.color = "#b42318";
        message.textContent =
          "Não foi possível solicitar a redefinição agora. Verifique sua conexão e tente novamente.";
        button.disabled = false;
      }
    };
    return true;
