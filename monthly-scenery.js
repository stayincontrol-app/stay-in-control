...
function apply(){const list=sceneList();if(!list.length)return;current=0;setBanner(list[current]);const bg=list[0].url;document.body.style.backgroundImage=`linear-gradient(rgba(244,247,251,.72),rgba(244,247,251,.72)),url("${bg}")`;document.body.style.backgroundPosition='center top';document.body.style.backgroundSize='cover';document.body.style.backgroundRepeat='no-repeat';document.body.style.backgroundAttachment='fixed';if(timer)clearInterval(timer);timer=setInterval(rotate,ROTATE_MS);installBackToLoginButton();loadScript('report-view.js','data-stay-report-view');loadScript('user-manager.js','data-stay-user-manager');loadScript('role-manager.js','data-stay-role-manager');loadScript('property-create-fix.js','data-stay-property-create-fix');}
