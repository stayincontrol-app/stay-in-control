...
let idx=0,currentId='',nextDue=0,closedUntil=0;
...
function tick(){hideInlineSponsored();const list=eligibleSponsored(),t=Date.now(),h=host();if(!list.length||t<closedUntil){h.classList.remove('is-visible');return}if(currentId&&t<nextDue){h.classList.add('is-visible');return}const b=list[idx%list.length];idx=(idx+1)%Math.max(1,list.length);currentId=String(b.id||'');nextDue=t+Math.max(3,Number(b.intervalSeconds)||Number(b.durationSeconds)||5)*1000;render(b)}
function boot(){ensureCss();host();setInterval(tick,1000);tick();window.addEventListener('t2-banners-updated',()=>{currentId='';nextDue=0;tick()})}
